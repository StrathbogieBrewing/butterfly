...
void Timer0_Init(void)
{
    ...
    // Sets the compare value for 10ms delay
#ifdef OSC_ALT
	OCR0A = 35;	// t0 is at 3600kHz
#else
	OCR0A = 38; // t0 is ~3900kHz
#endif
	
	// Set Clear on Timer Compare (CTC) mode, CLK/256 prescaler
	//	TCCR0A = (1<<WGM01)|(0<<WGM00)|(4<<CS00); // 1Mhz Clk 
	
	// Set Clear on Timer Compare (CTC) mode, CLK/1024 prescaler
	TCCR0A = (1<<WGM01)|(0<<WGM00)|(5<<CS00); // 4Mhz Clk
}
