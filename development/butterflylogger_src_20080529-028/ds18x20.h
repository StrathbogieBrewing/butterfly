...
#define MAXSENSORS DS1820_COUNT

//Strip down code when only 1 sensor NAL
// undefine to use both 
#define SENSOR_ID	DS18B20_ID
...
extern void ds18x20_print(uint8_t nSensor);
extern uint8_t ds18x20_read( uint8_t nSensor, uint8_t *subzero, uint8_t *cel, uint8_t *cel_frac_bits);
extern void DS18x20_uart_put_temp(const uint8_t subzero, uint8_t cel, uint8_t cel_frac_bits);

#if MAXSENSORS>1
extern uint8_t gnSensors;
#endif
